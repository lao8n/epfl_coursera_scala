...
class KMeans extends KMeansInterface {
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] = {
    if(points.isEmpty && !means.isEmpty) Map[Point, Seq[Point]]((means.head, Seq()))
    else points.groupBy(p => findClosest(p, means))
  }

  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    if(points.isEmpty && !means.isEmpty) ParMap[Point, ParSeq[Point]]((means.head, ParSeq()))
    else points.groupBy(p => findClosest(p, means))  
  }
  ...
  // classified is a map of each mean to the sequence of points in the corresponding cluster
  // input of oldMeans
  // output of newMeans
  def update(classified: Map[Point, Seq[Point]], oldMeans: Seq[Point]): Seq[Point] = {
    oldMeans map {
      oldMean: Point => findAverage(oldMean, classified(oldMean))
    }
  }
